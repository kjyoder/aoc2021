...
```{r}
mwin <- function(d, win_length=2) {
  n_win <- length(d) - win_length + 1
  out <- vector(mode='list', length=n_win)
  for (i in 1:n_win) {
    out[[i]] <- d[i:(i+win_length-1)]
  }
  return(out)
}

apply_rule <- function(probe, rules, last=FALSE) {
  # merge probe into single string
  probe_str <- paste0(probe, collapse='')
  insert <- rules %>%
    filter(input == probe_str) %>%
    select(out) %>% pull()
  if (last)
    return(c(probe[1], insert, probe[2]))
  else
    return(c(probe[1], insert))
}

apply_rules <- function(input, rules, n=1, ...) {
  for (i in 1:n) {
    # window the input
    win <- mwin(d, ...)
    # insert new characters for each window
    new_str <- NULL
    for (wi in 1:length(win)) {
      if (wi == length(win)) {
        inserted <- apply_rule(win[[wi]], rules, last=TRUE)
      } else {
        inserted <- apply_rule(win[[wi]], rules)
      }
      new_str <- append(new_str, inserted)
    }
    d <- new_str
  }
  return(new_str)
...
```{r}
polymer <- apply_rules(d, rules, n=10)

```

```{r}
pd <- tibble(char = polymer)
counts <- pd %>% count(char) %>% arrange(n)
counts[nrow(counts),2] - counts[1,2] 
```

## Part 2 - 40 steps

Yikes, brute forcing this is....not possible. But maybe we can learn about growth by looking at a few steps.


First, Let's optimize rule application within tidyverse
```{r}
count_letters <- function(d) {
  p <- tibble(char = d)
  return(p %>% count(char))
}

move_win <- function(d, win_length=2) {
  n_win <- length(d) - win_length + 1
  out <- vector(mode='list', length=n_win)
  for (i in 1:n_win) {
    out[[i]] <- d[i:(i+win_length-1)]
...
apply_rule2 <- function(d, R=rules) {
  d <- d %>% 
    unite(probe, A, B, remove=FALSE, sep='') %>% 
    mutate(new = R$out[match(.$probe, R$input)]) %>%
    select(A, new, B)
  return(d)
}

tb2char <- function(d) {
  # convert the tibble to a list of characters
  f_char <- d[[1,1]]
  chars <- d %>% unite(c, new, B, sep='') %>%
    select(-A) %>%
    pull() %>%
    paste0(collapse='')
  chars <- paste0(f_char, chars, collapse='') %>% 
    strsplit("") %>% 
    unlist()
  return(chars)
}

apply_rules2 <- function(d, n=1) {
  for (i in 1:n) {
    d <- d %>%
      move_win() %>%
      apply_rule2() %>%
...
for (i in 1:10) {
  ...
  polymer2 <- polymer2 %>%
      ...
      apply_rule2() %>%
      tb2char()
  pc <- polymer2 %>% count_letters()
  h <- append(h, pc$n[pc$char == 'H'])
  b <- append(b, pc$n[pc$char == 'B'])
}
print(h)
print(b)
```
...
What we want to do instead is just keep track of pairs and number of letters. The `rules` tell us how many letters of each type to add, and we can just just a little bit of logic to know how many pairs to add.

```{r}
# How many distinct letters might we see?
length(unique(rules$out))
```

`10` isn't so many

```{r}
# What are the letters in alphabetical order?
unique(rules$out) %>% sort()
```

```{r}
new_rules <- rules %>%
  separate(input, into=c('A', 'B'), sep=1:2) %>%
  mutate(new1 = str_c(A,out),
         new2 = str_c(out,B)) %>%
  unite(input, A, B, sep='')

counts <- rep(0, 10)

pair_labels <- unique(new_rules$input) %>% sort
pair_dict <- tibble(pairs=pair_labels) %>%
  rowid_to_column()
pair_dict %>% head()
```
...
```{r}
initialize_pairs <- function(x, pair_counts, pdict=pair_dict) {
  c <- x %>% count(pair)
  pair_idx <- match(c$pair, pdict$pairs)
  pair_counts[pair_idx] <- pair_counts[pair_idx] + c$n
  return(pair_counts)
}

pc2pairs <- function(pair_counts, pair_dict) {
  pairs <- NULL
  for (i in 1:length(pair_counts)) {
    if (pair_counts[i] > 0) {
      pairs <- append(pairs, rep(pair_dict$pairs[i], pair_counts[i]))
    }
  }
  return(pairs)
}

add_pairs <- function(pair_counts, pdict=pair_dict, rules=new_rules) {
  new_counts <- rep(0, length(pair_counts))
  # track relevant pair based on new pairs
  for (i in 1:length(pair_counts)) {
    np <- pair_counts[i]
    if (np > 0) {
      # lookup index in pair_dict
      pid <- pdict[i,]
      
      #print(str_c('Found pair at [',i,']: ', pid$pairs))
      
      new <- new_rules %>%
        filter(input == pid$pairs)
          
      new1 <- new$new1
      new2 <- new$new2
      #print(str_c('Adding new pairs: ',new1,' and ',new2))
      
      idx1 <- match(new1, pdict$pairs)
      idx2 <- match(new2, pdict$pairs)
      
      # get index for new pairs
      new_counts[idx1] <- new_counts[idx1] + np
      new_counts[idx2] <- new_counts[idx2] + np
...
count_letters <- function(pair_counts, pdict=pair_dict) {
  
  letter <- tibble('B'=1, 'C'=2, 'F'=3, 'H'=4, 'K'=5,
                   'N'=6, 'O'=7, 'P'=8, 'S'=9, 'V'=10)
  letter_counts <- rep(0, length(letter))
  
  for (i in 1:length(pair_counts)) {
    if (pair_counts[i] > 0) {
      # find characters
      pair <- pdict[i,2][[1]]
      letters <- strsplit(pair,'')
      idx1 <- letter[letters[[1]][1]]
      idx2 <- letter[letters[[1]][2]]
      letter_counts[idx1[[1]]] <- letter_counts[idx1[[1]]] + pair_counts[i]
      letter_counts[idx2[[1]]] <- letter_counts[idx2[[1]]] + pair_counts[i]
    }
  }
  # add one to B and one to K as first and last characters
  letter_counts[1] <- letter_counts[1] + 1
  letter_counts[5] <- letter_counts[5] + 1
  counts <- tibble(letter = names(letter),
                   count = letter_counts/2)
  return(counts %>% arrange(letter))
}
```


```{r}
# intitialize list of pairs
dw <- d %>% mwin()
cp <- NULL
for (i in 1:length(dw)) 
  cp <- append(cp, paste0(dw[[i]], collapse=''))
dp <- tibble(pair = cp)
dp %>% head()
```

```{r}
#initialize after one step for testing
dw2 <- d %>% move_win() %>% apply_rule2() %>% tb2char() %>% mwin()
cp2 <- NULL
for (i in 1:length(dw2)) 
  cp2 <- append(cp2, paste0(dw2[[i]], collapse=''))
dp2 <- tibble(pair = cp2)
```


```{r}
pair_counts <- rep(0, nrow(pair_dict))
pc <- initialize_pairs(dp, pair_counts)
```
...
# Day 15: Chiton
Pathing through a big matrix, adding the `risk_level` of each step as you enter the space
```{r}
require(tidyverse)
d <- read_csv('data/aoc-15.txt', col_names='val',
              col_types=cols(.default=col_character()))
dm <- d %>% 
  separate(val, into=str_c('c',1:100), sep=1:100) %>%
  mutate_all(as.integer) %>%
  as.matrix()
dim(dm)
```

Let's a make class to act as a navigator through the space

We want to start in the upper right (row=1, col=1)
Then move towards lower-value areas until we get to (row=100, col=100)

```{r}
navs <- list()

navs[[1]] <- list('nid'=1, 'mat'=dm, 'searched'=dm*0, 'risk'=0,
                  'position'=NULL, 'xrd'=FALSE, 'finished'=FALSE)
class(navs[[1]]) <- 'Navigator'

init <- function(object, ...)    { UseMethod('init') }
ping <- function(object, ...)    { UseMethod('ping') }
step <- function(object, ...)    { UseMethod('step') }
step_to <- function(object, ...) { UseMethod('step_to') }
clones <- function(object, ...) { UseMethod('clones') }
report <- function(object, ...)  { UseMethod('report') }
summary <- function(object, ...) { UseMethod('summary') }

init.Navigator <- function(object, nid=1, risk=0, row=1, col=1, searched=1) {
  object$position <- c('row'=row, 'col'=col)
  object$searched[searched] <- 1
  object$mat[searched] <- -1
  object$risk <- risk
  return(object)
}

ping.Navigator <- function(object, allow_diag=FALSE) {
  p <- object$position
  pr <- p['row']
  pc <- p['col']
  m <- object$mat
  s <- object$searched
  res <- matrix(rep(-1,9), nrow=3)
  
  dirs <- tibble(row = c(pr, pr+1),
                 col = c(pc+1, pc))
  if (allow_diag) {
    t2 <- tibble(row = c(pr+1),
                 col = c(pc+1))
    dirs <- dirs %>%
      bind_rows(t2)
  }
  dirs <- dirs %>%
    filter(row > 0 & row < nrow(m)) %>%
    filter(col > 0 & col < ncol(m))
  res <- tibble(id = rep(0, nrow(dirs)),
                risk = rep(0, nrow(dirs)))
  for (i in 1:nrow(dirs)) {
    id <- rc2i(dirs[i,]$row, dirs[i,]$col)
    res[i,]$id <- id
    res[i,]$risk <- m[id]
  }
  res <- res %>%
    filter(! id %in% which(s==1))
  return(res)
}

step.Navigator <- function(object, row=0, col=0) {
  p <- object$position
  p['row'] <- p['row'] + row
  p['col'] <- p['col'] + col
  id <- rc2i(p['row'], p['col'])
  object$risk <- object$risk + object$mat[p['row'], p['col']]
  object$position <- p
  object$mat[id] <- -1
  object$searched[id] <- 1
  
  if (p['row'] == nrow(object$mat) & p['col'] == ncol(object$mat)) {
...
step_to.Navigator <- function(object, target) {
  targetrc <- target %>% i2rc()
  object$position['row'] <- targetrc['row']
  object$position['col'] <- targetrc['col']
  object$risk <- object$risk + object$mat[target]
  object$mat[target] <- -1
  object$searched[target] <- 1
  
  if (target == rc2i(100, 100)) {
    object$finished <- TRUE
  }
...
report.Navigator <- function(object, ...) {
  return(object$risk)
}


clones.Navigator <- function(object, ...) {
  # return a list of Navigator objects,
  # one for each possible split, with ids
  # <root_id>_<ordinal>
  root_id <- rc2i(object$position['row'],
                  object$position['col'])
  ...
  out <- list()
  print('Generating clones...')
  for (i in 1:nrow(minrisk)) {
    print(str_c('Jumping from ', root_id,
                ' to ', minrisk[i,]$id))
    nv <- object %>%
      step_to(minrisk[i,]$id)
    nv$nid <- str_c(root_id,'_',i)
    out[[i]] <- nv
  }
...
summary.Navigator <- function(object) {
  print(str_c('Navigator: ', object$nid))
  print(str_c('Currently at [', object$position['row'],
              ', ', object$position['col'], '] AKA cell=', 
              rc2i(object$position['row'], object$position['col'])))
  print(str_c('After searching ', sum(object$searched)-1,' risk is: ', object$risk))
  if(object$finished) {
    print('Navigator has reached the exit')
  } else {
    print('Navigator has NOT reached the exit')
  }
}


# we can recycle some from Day 9
i2rc <- function(x, nrow=100) {
  row <- x %% nrow
  if (row == 0) {
    row <- 10
  }
...
# search_neighbor <- function(mat, r, c) {
#   rmax <- nrow(mat)
#   cmax <- ncol(mat)
#   
#   if (mat[r,c] == 0 || mat[r,c] == -1) return(mat)
...
```{r}
find_xroad <- function(nv) {
  
  while (!nv$finished) {
    #print(i)
    #print(nv$position)
    #print(length(which(nv$search > 0)))
    options <- nv %>% ping()
    if (nrow(options) == 0) {
      nv$finished <- TRUE
      return(nv)
    ...
    if (nrow(minrisk) == 1) {
      # great, only one choice
      minrc <- i2rc(minrisk[1,]$id)
      if (minrc['row'] == nv$position['row']) {
        nv <- nv %>% 
          step(col=minrc['col'] - nv$position['col'])
      } else {
        nv <- nv %>% 
          step(row=minrc['row'] - nv$position['row'])
      }
    } else {
      # we've got a split
      nv$xrd <- TRUE
...
Hmm...don't think this is gonna work. Let's try Djikstra instead
https://en.wikipedia.org/wiki/Dijkstra%27s_algorithm


```{r}
check_neighbor <- function(cur_id, n_id, 
                           td=dist_mat, risk=dm) {
  # current node
  cur_val <- td[cur_id]
  # neighbor value
  nb_val <- td[n_id]
  # proposed val
  prop <- cur_val + risk[n_id]
  
  if (prop < nb_val) {
    td[n_id] <- prop
  }
  
  out <- list('dist'=td, 'risk'=risk)
}

check_neighbors <- function(cur_id, visited, 
                            td=dist_mat, risk=dm,
                            nrow=100) {
  ...
  if (r < nrow(td)) nbrs <- append(nbrs, rc2i(r+1, c, nrow))
  if (c < ncol(td)) nbrs <- append(nbrs, rc2i(r, c+1, nrow))
  
  for (ni in 1:length(nbrs)) {
    res <- check_neighbor(cur_id, nbrs[ni],
                          td, risk)
    td <- res$dist
  }
  visited[cur_id] <- 1
...
n_row <- nrow(dm)
n_col <- ncol(dm)
visited <- matrix(rep(0,n_row*n_col), nrow=n_row)
dist_mat <- matrix(rep(Inf,n_row*n_col), nrow=n_row)
dist_mat[1] <- 0
...
while(any(visited == 0)) {
  nbrs_to_check <- NULL
  # print(str_c('This loop we need to check ', length(nbrs),
  #             ' nodes'))
  for (i in 1:length(nbrs)) {
    res <- check_neighbors(nbrs[i], visited,
                           dist_mat, dm, n_row)
    nbrs_to_check <- append(nbrs_to_check, res$nbrs)
    visited <- res$visited
    dist_mat <- res$dist
  }
  if (length(nbrs_to_check) > 0) {
    nbrs <- unique(nbrs_to_check)
  } else {
    break
  }
}
...
## Day 15 Part 2: much bigger map

```{r}
increment_tile <- function(mat, times=1) {
  mat <- mat + 1
  mat[mat>9] <- 1
  ...
  if (times == 0)
    ...
    return(increment_tile(mat, times))
}

m1 <- dm
m2 <- dm %>% increment_tile()
m3 <- dm %>% increment_tile(2)
m4 <- dm %>% increment_tile(3)
m5 <- dm %>% increment_tile(4)
m6 <- dm %>% increment_tile(5)
m7 <- dm %>% increment_tile(6)
m8 <- dm %>% increment_tile(7)
m9 <- dm %>% increment_tile(8)

r1 <- cbind(m1, m2, m3, m4, m5)
r2 <- cbind(m2, m3, m4, m5, m6)
r3 <- cbind(m3, m4, m5, m6, m7)
r4 <- cbind(m4, m5, m6, m7, m8)
r5 <- cbind(m5, m6, m7, m8, m9)

DM <- rbind(r1, r2, r3, r4, r5)
```

Ummm...I guess just try with the bigger input?

```{r}
# initial matrix is dm
n_row <- nrow(DM)
n_col <- ncol(DM)
visited <- matrix(rep(0,n_row*n_col), nrow=n_row)
dist_mat <- matrix(rep(Inf,n_row*n_col), nrow=n_row)
...
```{r}
require(tidyverse)
d <- read_csv('data/aoc-16.txt', col_names='packet',
              col_types = cols(.default=col_character()))
```
...
```{r}
read_packet <- function(chars, start_id = 1, char_id=char_ids) {
  
  # first 3 are version
  ver <- bin2dec(chars[1:3 + (start_id-1)])
  # next 3 are type
  typ <- bin2dec(chars[4:6 + (start_id-1)])
  
  length_id <- NA
  sub_packet <- NA
  # if type is 4, we're dealing with a literal
  # if type is not 4, it's an operator
  idx <- 7 + (start_id-1)
  
  if (typ == 4) {
    lbl <- "Simple number"
    header_end <- idx-1
    # grab 5 bits at a time until you get a 5-bit chunk that starts with a 0
    val <- NULL
    bits <- chars[idx:(idx+4)]
    while (bits[1] == 1) {
      val <- append(val, bits[2:5])
      idx <- idx + 5
      bits <- chars[idx:(idx+4)]
    }
    val <- append(val, bits[2:5])
    ...
    body_end <- idx - 1
    packet_info <- bin2dec(val)
  } else {
    # it's an operator
    
    # get the length id
    length_id <- bin2dec(chars[idx])
    idx <- idx + 1
    
    if (length_id == 0) {
      # the next 15 bits are a single number
      #   number of bits included in subpacket
      num <- idx:(idx+14)
      idx <- idx+15
      header_end <- idx-1
      sub_length <- bin2dec(chars[num])
      
      lbl <- str_c('Operator: 15-bits. ', sub_length,' bits in this packet')
      packet_info <- sub_length
      sub_packet <- chars[idx:(idx+sub_length-1)]
      body_end <- idx+sub_length-1
    } else {
      # length_id is 1, meaning next 11 bits indicate
      # the number of subpackets
      num <- idx:(idx+10)
      idx <- idx+11
      header_end <- idx-1
      sub_length <- bin2dec(chars[num])
      lbl <- str_c('Operator: n=',sub_length,' 11-bit number(s)')
      
      vals <- vector(mode='numeric', length=3)
      for (si in 1:sub_length) {
        num <- idx:(idx+10)
        vals[si] <- bin2dec(chars[num])
        idx <- idx+11
      }
      ...
      body_end <- idx-1
      #sub_packet <- bin2dec(sub_packet)
    }
  }
  remaining <- 0
  if (idx != length(chars)) {
    remaining <- length(chars) - idx
  }
  
  # make sure body end is at the end of a byte
  while (char_id[body_end] %% 4 != 0) {
    body_end <- body_end + 1
  }
  
  hex_str <- NULL
  for (i in 1:(body_end/4)) {
    hex_str <- append(hex_str, bin2hex(
      chars[ ((i*4)-3):(i*4)  ] ))
  }
  
  out <- list('version'=ver, 'type'=typ, 'length_id'=length_id, 
              'val'=packet_info, 'sub_packets'=sub_packet,
              'start_id'=start_id,
              'header_end'=header_end, 'body_end'=body_end,
              'remain'=remaining, 'label'=lbl, 
              'hexstr'= paste0(hex_str, collapse='')   )
  
  return(out)
}

get_subpackets <- function(packet) {
  n_packets <- packet$val
  p_done <- 0
  while (p_done < n_packets) {
    p <- read_packet()
...
```{r}
# all_packets <- list()
# processed_chars <- rep(0, length(chars))
# all_packets <- append(all_packets, read_packet(chars))
# if (all_packets[[length(all_packets)]]$type == 4) {
#   processed_chars
# }
```

```{r}
char_ids <- 1:length(chars)
# get first packet
start_id <- 1
p1 <- read_packet(chars, start_id)
pchars <- p1$sub_packets
p1

start_id <- p1$header_end+1
p2 <- read_packet(chars, start_id)
p2

start_id <- p2$body_end+1
p3 <- read_packet(chars, start_id)
p3
```

```{r}
p2 <- read_packet(pchars)
p2
pchars <- pchars[ (p2$body_end+1) : length(pchars)]

p3 <- read_packet(pchars)
pchars <- pchars[ (p3$body_end+1) : length(pchars)]
p3

p4 <- read_packet(pchars)

# there are 6 subpackets
sub_packs <- vector(mode='list', length=6)
p_end <- length(pchars)
un_proc <- pchars[(p2$body_end+1):p_end]
u_end <- length(un_proc)

p2.1 <- read_packet( un_proc )

p2.2 <- read_packet (un_proc[(p2.1$body_end+1):u_end])

p2.2 <- read_packet( pchars[p2.1$next_idx : p_end ] )
p2.3 <- read_packet( pchars[p2.2$next_idx : p_end ] )
p2.4 <- read_packet( pchars[p2.3$next_idx : p_end ] )
p2.5 <- read_packet( pchars[p2.4$next_idx : p_end ] )
p2.6 <- read_packet( pchars[p2.5$next_idx : p_end ] )
```