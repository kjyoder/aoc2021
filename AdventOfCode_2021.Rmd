...
navs[[1]] <- list('nid'=1, 'mat'=dm, 'searched'=dm*0, 'risk'=0, 'position'=NULL, 'finished'=FALSE)
...
ping.Navigator <- function(object, allow_diag=FALSE) {
  ...
  dirs <- tibble(row = c(pr, pr, pr-1, pr+1),
                 col = c(pc+1, pc-1, pc, pc))
  if (allow_diag) {
    t2 <- tibble(row = c(pr-1, pr+1, pr-1, pr+1),
                 col = c(pc-1, pc-1, pc+1, pc+1))
    dirs <- dirs %>%
      bind_rows(t2)
  }
  dirs <- dirs %>%
...
step_to.Navigator <- function(object, target) {
  ...
  positionrc <- object$position 
  
  if (targetrc['row'] == positionrc['row']) {
    object <- object %>% step(col = targetrc['col'] - positionrc['col'])
  } else {
    object <- object %>% step(row = targetrc['row'] - positionrc['row'])
  }
...
find_xroad <- function(nv) {
  ...
  for (i in 1:1e4) {
    print(i)
    #print(nv_test$position)
    #print(length(which(nv_test$search > 0)))
    options <- nv_test %>% ping()
    minrisk <- options %>% 
      filter(risk == min(risk))
    if (length(minrisk) == 1) {
      # great, only one choice
      minrc <- i2rc(minrisk[1,]$id)
      if (minrc['row'] == nv_test$position['row']) {
        nv_test <- nv_test %>% 
          step(col=minrc['col'] - nv_test$position['col'])
      } else {
        nv_test <- nv_test %>% 
          step(row=minrc['row'] - nv_test$position['row'])
      }
    } else {
      # we've got a split
      break
...
```{r}
nv <- list('nid'=1, 'mat'=dm, 'searched'=dm*0, 'risk'=0, 'position'=NULL, 'finished'=FALSE)
class(nv) <- 'Navigator'

nv_test <- nv %>% init()
nv_xroads <- nv %>% find_xroad()
nv_xroads %>% ping() 
```
