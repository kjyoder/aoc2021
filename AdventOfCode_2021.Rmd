...
Now that we have a list of low-point locations in the array, we can use those for starting points to build up basins.
```{r}
# keep track of indices for each basin
basins <- vector(mode='list', length=length(low_points))

#for (li in 1:length(low_idxs)) {
#  nodes <- 1
#  
#}
```

Wait, we don't actually care where the lowpoints are, we just want to find basins

```{r}
ridges <- ifelse(dm > 8, 1, 0)
