...
```{r}
#
push_flash <- function(mat, id) {
  # convert to row and column
  frow <- id %% nrow(mat)
  fcol <- ceiling(id / nrow(mat))
  
  # increment surrounding octopi
  frows <- c(frow-1, frow, frow+1)
  fcols <- c(fcol-1, fcol, fcol+1)
...
check_flash <- function(mat, flashes=0, flashed=0) {
  ...
  if (length(flashing) == 0) {
    out <- list('mat'=mat, 'flashes'=flashes)
    return(out)
  }
  ...
  if (length(flash2)==0) {
    flashes <- flashes + length(flash2)
    out <- list('mat'=mat, 'flashes'=flashes)
    return(out)
  } else {
    mask <- flashing
    new <- check_flash(mat, flashes=flashes)
    new$mat[mask] <- 0
    return(new)
  }
}
...
step_multiple <- function(mat, flashes=0, steps=1) {
